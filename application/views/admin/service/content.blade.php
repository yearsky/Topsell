...
@section('title')
{{($type=="create") ? 'Buat Service Baru' : 'Ubah Service' }} 
@endsection
...
<div class="page-header">
	...
	<div class="breadcrumb-line">
		<ul class="breadcrumb">
			<li><a href="{{base_url('superuser/service')}}"><i class="icon-archive position-left"></i> Service</a></li>
...
<div class="content">
	<h6 class="content-group text-semibold">
		<span class="text-primary"><i class="icon-archive"></i> {{ ($type=="create") ? 'Buat' : 'Ubah' }}</span> Service
		<small class="display-block">
			{{ ($type=="create") ? 'Buat Service Baru' : 'Perbarui Service' }}
		</small>
	</h6>
	<div class="row">
		<div class="col-lg-12">
			<div class="panel panel-white">
				...
				<div class="col-md-12 clearfix" style="margin-top:10px;margin-bottom:10px;float:none;padding:20px;">
					<form id="form-blog" class="form-horizontal" 
						action="{{ ($type=='create') ? base_url('superuser/service/created') : base_url('superuser/service/updated/'.$service->id) }}" 
					method="post">
						<div class="form-group">
							...
							<div class="col-lg-10">
								...
								value="{{ ($type=='create') ? '' : $service->name }}" required>
							</div>
						</div>
						...
						<div class="form-group">
							...
							<div class="col-lg-10">
								<textarea  rows="3"  name="description_sort" class="form-control" placeholder="Deskripsi pendek maksimal 150 karakter" maxlength="150">{{ ($type=='create') ? '' : $service->description_sort }}</textarea>
							</div>
						</div>
						...
						<div class="form-group">
							...
							<div class="col-lg-10">
								<textarea  id="editor-full" rows="2"  name="description" cols="2">{!! ($type=='create') ? '' : $service->description !!}</textarea>
							</div>
						</div>
						...
						<div class="form-group">
							...
							<div class="col-lg-10">
								<div class="checkbox checkbox-switch">
									<label>
										<input type="checkbox" name="status" value="0" class="switch" data-on-text="Publish" data-off-text="Draft" data-on-color="success" data-off-color="default" {{ (@$service->status==0) ? 'checked' : '' }} >
						...
						<div class="text-right">
							<button type="submit" class="btn btn-primary">{{ ($type=='create') ? 'Buat Service' : 'Ubah Service' }} <i class="icon-arrow-right14 position-right"></i></button>
							@if($type=="update")
...
<script type="text/javascript">
	...
	$('.file-input-custom').fileinput({
        ...
        initialPreview: ["<img src='{{ ($type=='create') ? img_holder() : $service->imagedir }}' class='file-preview-image' alt=''>",],
	...
	$("#form-blog").submit(function(e){
			...
			$.ajax({
				...
				data:  		new FormData(this),
          		...
          		contentType: false,
				beforeSend: function(){
					blockMessage($('#form-blog'),'Please Wait , {{ ($type =="create") ? "Menambahkan Service" : "Memperbarui Service" }}','#fff');		
				}
			})
			.done(function(data){
				$('#form-blog').unblock();
				...
				function(){
					if(data.auth!=false){
						redirect("{{base_url('superuser/service')}}");		
						return;
					}
				});
