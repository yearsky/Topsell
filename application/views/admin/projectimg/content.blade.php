...
@section('title')
{{($type=="create") ? 'Buat Gallery Baru' : 'Ubah Gallery' }} - Administrasi
@endsection
...
<div class="page-header">
	...
	<div class="breadcrumb-line">
		<ul class="breadcrumb">
			...
			<li class="active">Buat Gallery</li>
		</ul>
	</div>
</div>
...
<div class="content">
	<h6 class="content-group text-semibold">
		...
		<small class="display-block">
			{{ ($type=="create") ? 'Buat Gallery Baru' : 'Perbarui Gallery' }}
		</small>
	</h6>
	<div class="row">
		<div class="col-lg-12">
				...
				<div class="col-md-12 clearfix" style="margin-top:10px;margin-bottom:10px;float:none;padding:20px;">
					...
					method="post">
						<div class="form-group">
							<label class="col-lg-2 control-label">Judul Gallery <span class="text-danger"><b>*</b></span></label>
							<div class="col-lg-10">
								<input class="form-control" type="text" placeholder="Judul Gallery" name="title" 
								value="{{ ($type=='create') ? '' : $gallery->title }}" required>
							</div>
						</div>
						...
						<div class="form-group">
							...
							<div class="col-lg-10">
								<div class="checkbox checkbox-switch">
									<label>
										<input type="checkbox" name="type" onchange="typeGallery(this)" value="picture" class="switch" data-on-text="Gambar" data-off-text="Video" data-on-color="primary" data-off-color="danger" {{(@$gallery->type=="picture") ? 'checked' : ''}} >
									</label>
								</div>
							</div>
						</div>
						<div id="picture" style="{{ (@$gallery->type=='picture') ? '' : 'display:none'}}">
							<div class="form-group">
								<label class="col-lg-2 control-label">Sampul Gallery<span class="text-danger"><b>*</b></span></label>
								<div class="col-lg-10">
						...
						<div id="video"  style="{{ (@$gallery->type!='picture') ? '' : 'display:none'}};margin-left: -9px;">
...
<script type="text/javascript">
	...
	$(document).ready(function(){
	        ...
	        initialPreview: ["<img src='{{ ($type=='create') ? img_holder() : $gallery->imagedir }}' class='file-preview-image' alt=''>",],
	...
	$("#form-blog").submit(function(e){
			...
			.done(function(data){
				$('#form-blog').unblock();
				sweetAlert({
					title: 	((data.auth==false) ? "Opps!" : '{{ ($type =="create") ? "Gallery Di Buatkan" : "Gallery Di Perbarui" }}'),
					text: 	data.msg,
				...
				function(){
					...
					//redirect('{{ ($type =="create") ?  base_url("administrasi/slider/add") : base_url("administrasi/slider/edit/".$gallery->id."/".seo($gallery->title))}}');
				});
			...
			.fail(function() {
				...
				function(){
					/*redirect('{{ ($type =="create") ?  base_url("administrasi/gallery/create") : base_url("administrasi/gallery/update/".$gallery->id."/".seo($gallery->title))}}');*/
				});
			 })
