...
@section('title')
{{($type=="create") ? 'Buat Menu Pengunjung Baru' : 'Ubah Menu Pengunjung' }} - Administrasi
@endsection
...
<div class="content">
	<h6 class="content-group text-semibold">
		<span class="text-primary"><i class="icon-magazine"></i> {{ ($type=="create") ? 'Buat' : 'Ubah' }}</span> Menu Pengunjung
		<small class="display-block">
			{{ ($type=="create") ? 'Buat Menu Pengunjung' : 'Perbarui Menu Pengunjung' }}
		</small>
	</h6>
	<div class="row">
		<div class="col-lg-12">
			<div class="panel panel-white">
				...
				<div class="col-md-12 clearfix" style="margin-top:10px;margin-bottom:10px;float:none;padding:20px;">
					<form id="form-blog" class="form-horizontal" 
						action="{{ ($type=='create') ? base_url('superuser/menupengunjung/created') : base_url('superuser/menupengunjung/updated/'.$menupengunjung->id) }}" 
					method="post">
						<div class="form-group">
							...
							<div class="col-lg-10">
								...
								value="{{ ($type=='create') ? '' : $menupengunjung->judul }}" required>
							</div>
						</div>
						<div class="form-group">
							...
							<div class="col-lg-10">
								...
								value="{{ ($type=='create') ? '' : $menupengunjung->link }}">
							</div>
						</div>
						<div class="form-group">
							...
							<div class="col-lg-10">
								...
								value="{{ ($type=='create') ? '' : $menupengunjung->urutan }}">
							</div>
						</div>
						...
						<div class="form-group">
							...
							<div class="col-lg-10">
								<div class="checkbox checkbox-switch">
									<label>
										<input type="checkbox" name="aktif" value="0" class="switch" data-on-text="Aktif" data-off-text="Tidak Aktif" data-on-color="success" data-off-color="default" {{(@$menupengunjung->aktif==0) ? 'checked' : ''}} >
										Aktif / Tidak Aktif
									</label>
								</div>
							</div>
						...
						<div class="text-right">
							<button type="submit" class="btn btn-primary">{{ ($type=='create') ? 'Buat Menu Pengunjung' : 'Ubah Menu Pengunjung' }} <i class="icon-arrow-right14 position-right"></i></button>
							@if($type=="update")
	...
	$("#form-blog").submit(function(e){
			...
			$.ajax({
				...
				data:  		new FormData(this),
          		...
          		contentType: false,
				beforeSend: function(){
					blockMessage($('#form-blog'),'Please Wait , {{ ($type =="create") ? "Menambahkan Menu Pengunjung" : "Memperbarui Menu Pengunjung" }}','#fff');		
				}
			})
			.done(function(data){
				$('#form-blog').unblock();
				sweetAlert({
					title: 	((data.auth==false) ? "Opps!" : '{{ ($type =="create") ? "Menu Pengunjung Di Buatkan" : "Menu Pengunjung Di Perbarui" }}'),
					text: 	data.msg,
					type: 	((data.auth==false) ? "error" : "success"),
				},
				function(){
					if(data.auth!=false){
						redirect("{{base_url('superuser/menupengunjung')}}");		
						return;
					}
					//redirect('{{ ($type =="create") ?  base_url("administrasi/slider/add") : base_url("administrasi/slider/edit/".$menupengunjung->id."/".seo($menupengunjung->title))}}');
				});
			...
			.fail(function() {
				...
				function(){
					/*redirect('{{ ($type =="create") ?  base_url("administrasi/menupengunjung/create") : base_url("administrasi/menupengunjung/update/".$menupengunjung->id."/".seo($menupengunjung->title))}}');*/
				});
			 })
